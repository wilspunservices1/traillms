const videoModal = () => {
  ...
  function playVideos(e) {
    ...
    // Check for MP4 or local video files
    else if (url.indexOf("mp4") !== -1 || url.indexOf("m4v") !== -1) {
      document.getElementById(
        "lvideo-wrap"
      ).innerHTML = `${startModal}<video controls loop playsinline autoplay><source src='${url}' type="video/mp4"></video>${finishModal}`;
    } 
    // Alert if no video link found
    else {
