    ...
    if (session?.user?.roles.includes('instructor')) {
      const fetchCourses = async () => {
        try {
          ...
          const response = await fetch(`${BASE_URL}/api/courses/intructorCourses?instructorId=${instructorId}`);
...
const submitCertificateForm = async (base64Image: string) => {
  try {
    ...
    const payload = {
      ...
      expirationDate: '2025-12-31T23:59:59.000Z', // Example expiration date
      isRevocable: true,
      metadata: JSON.stringify({
        courseName: selectedCourse?.label || '',
        instructor: session?.user?.name || '',
        courseDuration: '3 months', // Example metadata value
      }),
    };
    ...
    const certificateId = responseData.certificate_id;
    router.push(`/dashboards/certificates/edit/${certificateId}`);
...
//       const responseData = await response.json();
//         //   console.log('Certificate saved successfully:', responseData);
//         showAlert('success','Certificate saved successfully')
//         // Navigate to the edit page for the created certificate
//         const certificateId = responseData.certificate_id; // Assuming the server returns the certificate_id
//         router.push(`/dashboards/certificates/edit/${certificateId}`);
