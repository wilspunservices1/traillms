...
const CartContextProvider = ({ children }) => {
	...
	const fetchCartFromDB = useCallback(async () => {
		if (!userId) return; // Prevent unnecessary fetch if userId is not available
	...
	}, [userId]); // `useCallback` ensures `fetchCartFromDB` remains stable unless `userId` changes
	...
	}, [fetchCartFromDB, userId]); // Now `fetchCartFromDB` is safely included
	...
	const deleteProductFromCart = async (cartId) => {
		...
		try {
			const response = await fetch(
				`/api/user/${userId}/cart?cartId=${cartId}`,
				{
					method: "DELETE",
				}
			);
